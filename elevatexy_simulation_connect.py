...
MODIFIED FOR SITL CONTROL FIXES + Firebase mode + MJPEG streaming
"""
...
# ================== FIREBASE CONFIG ==================
SECRET_TOKEN = "elevatexy_secure_2025_abc123"

FIREBASE_API_KEY    = "AIzaSyC0n6ZSSnoXQ9d2Pdc-KF2dU3WIf5KCaSM"
FIREBASE_PROJECT_ID = "elevate-2-0-07zs58"
FIREBASE_EMAIL      = "device-nano01@yourapp.dev"   # same as your other script
FIREBASE_PASSWORD   = "BMVjjkjh2350!"

RTDB_URL = "https://elevate-2-0-07zs58-default-rtdb.firebaseio.com"
IDP = f"https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key={FIREBASE_API_KEY}"
TOK = f"https://securetoken.googleapis.com/v1/token?key={FIREBASE_API_KEY}"

DEVICE_KEY = "RnCq5z9aT2PjDWUX2LCQC4XXTrN2"
# HTTP streaming config
HTTP_HOST, HTTP_PORT = "0.0.0.0", 5001

# HTTP server + Firebase session
app = Flask(__name__)
...
def signin_firebase():
   """Sign in the device user (for future Firestore/etc)."""
   global id_token, refresh_token, device_id, token_exp_ts
   try:
       ...
       # For RTDB we want the fixed key (data), not localId
       device_id = DEVICE_KEY
       token_exp_ts = time.time() + 55 * 60
       print(f"✓ Firebase auth OK, using RTDB device_id = {device_id}")
   except Exception as e:
       # Don’t kill the whole program if auth fails
       print(f"⚠️ Firebase signin failed: {e}")
       device_id = DEVICE_KEY
       id_token = None
       refresh_token = None
       token_exp_ts = 0.0
...
def refresh_firebase_token():
   ...
   global id_token, refresh_token, token_exp_ts
   r = session.post(
       TOK,
...
def ensure_token():
   """Make sure id_token is valid before any RTDB call."""
   if not id_token or time.time() >= token_exp_ts:
       refresh_firebase_token()

def get_status_and_mode_from_firestore(
   default_status="manual",
   default_mode="standard",
):
   """
   Read control status + flight mode from Firestore.

   Firestore doc:
     devices/{DEVICE_KEY}
       status:       "manual" | "autonomous"
       mode:         "eco" | "standard" | "performance"
   """
   FIRESTORE_URL = (
       f"https://firestore.googleapis.com/v1/"
       f"projects/{FIREBASE_PROJECT_ID}/databases/(default)/documents/"
       f"devices/{DEVICE_KEY}?key={FIREBASE_API_KEY}"
   )

   try:
       r = requests.get(FIRESTORE_URL, timeout=3)
       if r.status_code != 200:
           print("⚠️ Firestore HTTP", r.status_code, r.text)
       ...
       # status
       status = (
           fields.get("status", {}).get("stringValue", default_status)
           or default_status
       )
       status = status.strip().lower()
       if status not in ("manual", "autonomous"):
           status = default_status

       # mode
       mode = (
           fields.get("mode", {}).get("stringValue", default_mode)
           or default_mode
       )
       ...
       print(f"⚠️ Firestore status/mode error: {e}")
       return default_status, default_mode


@app.route("/stream")
def stream():
   """MJPEG stream endpoint: /stream?token=SECRET"""
...
def index():
   ip = get_local_ip()
   stream_url = f"http://{ip}:{HTTP_PORT}/stream?token={SECRET_TOKEN}"
   return f"""
   <html><body>
   <h3>ElevateXY Simulation Stream</h3>
   <p>Use this URL in FlutterFlow (Network Image):</p>
   <code>{stream_url}</code>
   <p><img src="{stream_url}" style="max-width: 640px;"></p>
   </body></html>
   """
...
class DroneState:
   def __init__(self):
       # already there
       self.status = "manual"        # "manual" / "autonomous"
       # NEW:
       self.mode = "eco"             # "eco" / "standard" / "performance"

       # optional cached values so you can see when things change
       self._last_status_print = None
       self._last_mode_print = None

   def set_status(self, new_status: str):
       if new_status not in ("manual", "autonomous"):
           return
       if new_status != self.status:
           self.status = new_status
           print(f"[FS] status -> {self.status}")

   def set_mode(self, new_mode: str):
       new_mode = new_mode.lower()
       if new_mode not in ("eco", "standard", "performance"):
           return
       if new_mode != self.mode:
           self.mode = new_mode
           print(f"[FS] mode -> {self.mode}")
...
class ElevateXYSimulation:
   def __init__(self, connection_string, baud=57600):
       ...
       # 🔴 Firebase / RTDB mode listener thread
       self._rtdb_thread = threading.Thread(
           target=self._firebase_mode_loop, daemon=True
       )
       self._rtdb_thread.start()
   ...
   def _firebase_mode_loop(self):
       """
       Background loop that polls Firestore ~4x/sec
       to switch between MANUAL/AUTONOMOUS and ECO/STANDARD/PERFORMANCE.
       """
       last_status = None
       last_mode = None

       while True:
           status, mode = get_status_and_mode_from_firestore(
               default_status="manual",
               default_mode="standard",
           )
           ...
           if status != last_status:
               ...
               if status == "autonomous":
                   if not self.autonomous_enabled:
                       print("[Firestore] Enabling AUTONOMOUS mode")
                   self.autonomous_enabled = True
               else:
                   if self.autonomous_enabled:
                       print("[Firestore] Switching to MANUAL mode")
                       self.stop_movement()
                   self.autonomous_enabled = False
           ...
           if mode != last_mode:
               ...
               last_mode = mode
               # This updates move_speed, vertical_speed, etc.
               self.set_flight_mode(mode)

           # Faster reaction, but still reasonable on quotas
           time.sleep(0.25)
   ...
   def start_camera(self):
       """Start camera using GStreamer, fallback to OpenCV if needed."""
       print("Initializing Jetson camera (GStreamer)...")
       # First try Jetson CSI cam via GStreamer
       try:
           ...
           if self.cap.start():
               ...
               for _ in range(20):
                   ...
                   if ret and frame is not None:
                       print(f"✓ GStreamer camera ready ({frame.shape})")
                       self.camera_active = True
                       return True
                   time.sleep(0.1)
               print("⚠️ GStreamer camera started but no frames, falling back to OpenCV...")
           else:
               print("⚠️ GStreamer camera failed to start, falling back to OpenCV...")
       except Exception as e:
           print(f"✗ GStreamer camera error: {e}")
           print("Falling back to OpenCV VideoCapture(0)...")

       # Fallback: standard webcam / USB cam
       try:
           ...
           for _ in range(20):
               ret, frame = self.cap.read()
               if ret and frame is not None:
                   print(f"✓ OpenCV camera ready ({frame.shape})")
                   self.camera_active = True
                   return True
               time.sleep(0.1)

           print("✗ OpenCV camera opened but no frames.")
           return False
       except Exception as e:
           print(f"✗ Camera failed completely: {e}")
           return False
   ...
   def run(self):
       ...
       try:
           while self.camera_active:
               ...
               # Update MJPEG streaming frame
               try:
                   ok, jpg = cv2.imencode(".jpg", display_frame)
                   if ok:
                       global latest_jpeg
                       with jpeg_lock:
...
Use /dev/ttyUSB0 (or /dev/ttyACM0) with --baud for real drone connection.
   ...
   # 🌐 Start HTTP MJPEG server in background
   def run_flask():
       ip = get_local_ip()
       print("\n" + "=" * 70)
       print(f"📡 MJPEG Stream: http://{ip}:{HTTP_PORT}/stream?token={SECRET_TOKEN}")
       print(f"🌐 Web UI:       http://{ip}:{HTTP_PORT}")
       print("=" * 70 + "\n")
